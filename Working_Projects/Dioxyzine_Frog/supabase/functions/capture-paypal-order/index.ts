...
Deno.serve(async req => {
  ...
  try {
    ...
    // One PayPal order can have multiple reservation rows.
    ...
    if (!reservations?.length) {
      throw new Error(
        'Stock reservation is missing or no longer available. Please restart checkout.',
      );
    }
    ...
    if (!allConsumed && !allActive) {
      throw new Error(
        'Stock reservation is in an invalid state. Please restart checkout.',
      );
    }

    if (
      allActive &&
      reservations.some(r => new Date(r.expires_at) <= new Date())
    ) {
      throw new Error(
        'Checkout reservation expired. Please restart checkout.',
      );
    }
