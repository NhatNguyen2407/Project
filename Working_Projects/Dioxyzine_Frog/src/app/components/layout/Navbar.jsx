          ...
          <div className="flex items-center justify-between h-20">
            <Link to="/" className="flex items-center space-x-3 group">
              <motion.div whileHover={{ scale: 1.05 }} className="relative">
                <img src="/src/assets/avatar.jpeg" alt="Dioxyzine Frog" className="w-12 h-12 rounded-full border-2 border-[var(--primary)] object-cover shadow-[0_0_15px_rgba(139,114,190,0.5)]" />
              </motion.div>
              <span className="text-3xl tracking-wide drop-shadow-[0_0_8px_rgba(139,114,190,0.6)]" style={{ fontFamily: "'Coiny', cursive", color: "var(--primary)", WebkitTextStroke: "1.5px white", paintOrder: "stroke fill" }}>
                Dioxyzine Frog
              </span>
            </Link>
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#1A1528] border border-[var(--border)] text-[var(--silver-gray)] hover:text-white hover:border-[var(--primary)] transition-all"
              >
          ...
          <motion.div initial={{ opacity: 0, x: '100%' }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: '100%' }} transition={{ type: 'spring', damping: 25 }} className="fixed inset-y-0 right-0 z-40 w-full max-w-sm bg-[var(--card)] border-l border-[var(--border)] shadow-2xl md:hidden">
            <div className="flex flex-col h-full pt-24 pb-8 px-6 space-y-6">
              <div className="flex justify-end mb-4">
                <button onClick={() => setLang(lang === 'vi' ? 'en' : 'vi')} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#1A1528] border border-[var(--border)] text-white">
                  <Globe className="w-4 h-4" />
              ...
              {navLinks.map((link, index) => {
                ...
                return (
                  <motion.div key={link.path} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }}>
                    <Link to={link.path} className={`block text-2xl font-medium transition-colors ${isActive ? 'text-[var(--primary)]' : 'text-foreground hover:text-[var(--primary)]'}`}>
                      {link.label}
                    </Link>
                  </motion.div>
                );
