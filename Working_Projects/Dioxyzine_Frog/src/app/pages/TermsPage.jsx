...
export function TermsPage() {
  ...
  const renderContent = () => {
    switch (currentPath) {
      case 'printing':
        return (
          <motion.div key="printing" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            <h2 className="text-3xl font-bold font-heading mb-6">Terms of Printing</h2>
            
            <div className="bg-card p-6 rounded-2xl border border-border shadow-sm">
              <h3 className="text-xl font-bold text-[var(--primary)] mb-3">1. Technical Notes</h3>
              <ul className="space-y-3 text-foreground font-medium list-disc list-inside">
            ...
            <div className="bg-card p-6 rounded-2xl border border-border shadow-sm">
              <h3 className="text-xl font-bold text-[var(--primary)] mb-3">2. Production Notes</h3>
              <ul className="space-y-3 text-foreground font-medium list-disc list-inside">
      ...
      case 'refund':
        return (
          <motion.div key="refund" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            <h2 className="text-3xl font-bold font-heading mb-6">Return & Refund Policy</h2>
            
            <div className="bg-card p-6 rounded-2xl border border-border shadow-sm">
              <h3 className="text-xl font-bold text-[var(--primary)] mb-3">Policy Details</h3>
              <ul className="space-y-3 text-foreground font-medium list-disc list-inside">
      ...
      case 'membership':
        return (
          <motion.div key="membership" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            ...
            <p className="text-xs text-muted-foreground mt-4 italic font-medium">*Note: The system counts your milestones based on successfully completed orders within your account.</p>
          </motion.div>
        );
      default:
  ...
  return (
    <div className="min-h-screen pt-28 pb-16 bg-background relative z-10">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="w-full md:w-72 flex-shrink-0">
            <div className="bg-card rounded-3xl p-4 border border-border shadow-md sticky top-28 flex flex-col gap-2">
              {tabs.map((tab) => (
                <NavLink
                  key={tab.id}
                  to={tab.path}
                  className={({ isActive }) => `
                    flex items-center gap-3 w-full p-4 rounded-2xl transition-all duration-300 text-left font-bold
                    ${isActive 
                ...
                >
                  ...
                  <span className="text-sm md:text-base flex-1">{tab.label}</span>
