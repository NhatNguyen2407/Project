...
const slides = [
  { title: 'Custom Plush Manufacturer', subtitle: 'Turn your artwork into premium plushies. OEM/POD & Global Shipping.', image: doll2d_banner },
  { title: 'Design • Produce • Ship', subtitle: 'From single prototypes to bulk manufacturing. We are your reliable B2B partner.', image: doll2d_banner02 },
  { title: 'Premium Quality & Low MOQ', subtitle: 'Strict quality control, white-label services, and seamless fulfillment for creators.', image: doll2d_banner03 }
];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-screen overflow-hidden">
        {slides.map((slide, index) => (
          <motion.div key={index} initial={{ opacity: 0 }} animate={{ opacity: currentSlide === index ? 1 : 0 }} transition={{ duration: 1 }} className="absolute inset-0" style={{ pointerEvents: currentSlide === index ? 'auto' : 'none' }}>
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-background/95 via-background/60 to-transparent z-10"></div>
            <img src={slide.image} alt="" className="w-full h-full object-cover opacity-90" />
            <div className="absolute inset-0 z-20 flex items-center pt-16">
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                <motion.div initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2, duration: 0.8 }} className="max-w-3xl bg-card/40 backdrop-blur-md p-6 sm:p-8 rounded-3xl border border-border shadow-xl">
                  {/* Sử dụng font Coiny béo tròn tự động thông qua class font-heading */}
                  <h1 className="font-heading text-4xl sm:text-5xl md:text-7xl font-bold mb-6 leading-tight drop-shadow-sm">
                    {slide.title}
                  </h1>
                  <p className="text-lg md:text-2xl text-foreground font-semibold mb-8">{slide.subtitle}</p>
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="flex items-center gap-4 bg-card/80 border border-border backdrop-blur-md px-5 py-3 rounded-full w-fit shadow-sm">
                      <span className="text-sm font-bold text-foreground whitespace-nowrap">Follow us on:</span>
      ...
      <section className="py-24 bg-transparent relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="font-heading text-4xl md:text-5xl mb-4 drop-shadow-sm">How Ordering Works</h2>
            <p className="text-lg text-muted-foreground font-semibold max-w-2xl mx-auto">Simple, transparent, and collaborative process from concept to final creation</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {steps.map((step, index) => (
              <div key={index} className="bg-card border border-border rounded-3xl p-8 shadow-sm hover:shadow-md transition-shadow h-full">
                {/* Background icon chuyển sang dùng màu Xanh Lá Ếch nhạt dịu mát */}
                <div className="w-16 h-16 rounded-2xl bg-secondary border border-border flex items-center justify-center text-[var(--primary)] mb-6 shadow-sm">
      ...
      <section className="py-24 bg-transparent relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="font-heading text-4xl md:text-5xl mb-4 drop-shadow-sm">Featured Products</h2>
            <p className="text-lg text-muted-foreground font-semibold max-w-2xl mx-auto">Explore our most popular customizable merchandise and handmade plushies</p>
          </div>
      ...
      {/* CTA Section */}
      <section className="py-24 bg-card border-t border-border relative overflow-hidden z-10 text-center">
